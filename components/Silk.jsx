...
// Both the entrance and page silk use the same animation phase.
let silkMotionStartedAt = null;

const SilkPlane = forwardRef(function SilkPlane({ uniforms, speed, onReady }, ref) {
  ...
  useFrame(() => {
    ...
    if (speed > 0) {
      ...
      silkMotionStartedAt ??= now;
      material.uniforms.uTime.value = (now - silkMotionStartedAt) * 0.0001;
    } else {
      material.uniforms.uTime.value = 0;
    }
    // The previous frame has been painted before announcing readiness.
    frames.current += 1;
...
function SilkClock({ speed }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      invalidate();
      if (speed > 0 || --warmup > 0) timer = setTimeout(tick, 1000 / 30);
    };
    const resume = () => {
      clearTimeout(timer);
      warmup = 2;
...
const Silk = ({ speed = 5, scale = 1, color = '#7B7481', noiseIntensity = 1.5, rotation = 0, lightMode = false, onReady }) => {
  ...
  return (
    <Canvas dpr={1} frameloop="demand" gl={{ antialias: false, alpha: false }}>
      <SilkClock speed={speed} />
      <SilkPlane ref={meshRef} uniforms={uniforms} speed={speed} onReady={onReady} />
    </Canvas>
  );
};
